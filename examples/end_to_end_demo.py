...
def main():
    ...
    logger.info(
        f"🎯 Training configuration: {config.max_samples:,} samples, {config.epochs} epochs"
    )
    ...
    try:
        ...
        print(
            f"👥 Dataset: {result['model_info']['num_users']:,} users, {result['model_info']['num_items']:,} items"
        )
    ...
    try:
        ...
        print(
            f"📊 Model ready: {model_info['total_parameters']:,} parameters on {model_info['device']}"
        )
        ...
        for user in users[:3]:
            for item in items[:2]:
                pred = loader.predict_single(
                    user, item, {"verified": 1, "review_length": 150, "hour": 14}
                )
                print(f"  User {user} → Item {item}: {pred:.4f}")

        print(f"\nRecommendations for user {users[0]}:")
        recommendations = loader.recommend_items(
            users[0], num_recommendations=5, context={"verified": 1, "hour": 14}
        )

        for i, rec in enumerate(recommendations, 1):
            print(
                f"  {i}. {rec['item_id']}: {rec['score']:.4f} (confidence: {rec['confidence']:.2f})"
            )
