...
class DLRMModelLoader:
    ...
    def load_from_training_output(self, model_dir: str = "models") -> bool:
        ...
        try:
            ...
            with open(mappings_file, "r") as f:
                ...
                self.reverse_user_mapping = {
                    int(v): k for k, v in self.user_mapping.items()
                }
                self.reverse_item_mapping = {
                    int(v): k for k, v in self.item_mapping.items()
                }
    ...
    def predict_single(
        self, user_id: str, item_id: str, context: Optional[Dict] = None
    ) -> float:
        """Generate prediction for a single user-item pair."""

        if not self.is_loaded():
            raise ValueError(
                "Model not loaded. Call load_from_training_output() first."
            )
        ...
        dense_tensor = torch.tensor(
            [dense_features], dtype=torch.float32, device=self.device
        )
    ...
    def predict_batch(
        ...
        for user_id, item_id, context in zip(user_ids, item_ids, contexts):
            ...
            if user_idx is None or item_idx is None:
                cold_start_mask.append(True)
                user_indices.append(0)  # Placeholder index
                item_indices.append(0)
            else:
                cold_start_mask.append(False)
                user_indices.append(user_idx)
                item_indices.append(item_idx)
            dense_features_batch.append(self._extract_dense_features(context))

        # If all cold start, return early
        if all(cold_start_mask):
        ...
        dense_tensor = torch.tensor(
            dense_features_batch, dtype=torch.float32, device=self.device
        )

        # Get predictions
        with torch.no_grad():
            predictions = self.model(user_tensor, item_tensor, dense_tensor)
            results = predictions.squeeze(-1).cpu().numpy().tolist()

        # Handle single prediction case (numpy scalar .tolist() returns float)
        if isinstance(results, float):
            results = [results]

        # Replace cold start predictions with 0.5
        for i, is_cold in enumerate(cold_start_mask):
            if is_cold:
                results[i] = 0.5
    ...
    def recommend_items(
        ...
        candidate_indices = [
            idx for idx in all_item_indices if idx not in exclude_indices
        ]
        ...
        # Get predictions
        with torch.no_grad():
            scores = self.model(user_tensor, item_tensor, dense_tensor).squeeze(-1)
        ...
        top_k_indices = torch.topk(
            scores, min(num_recommendations, len(scores))
        ).indices
